...
def gen_image(output_path, font_path, mode, dict_file, quality):
    if quality == 'q2':
        ...
        font_size = 10
    days_in_week_list = [[[], [['1 - (8.30-10.00)']], [['2 - (10.10-11.40)']], [['3 - (12.40-14.10)']], [['4 - (14.20-15.50)']], 
    ...
    # print('width -', final_image_width,'height -', final_image_height)
...
def main():
    ...
    # group = 'БТПб 1-1'
    qualities = ['q1', 'q2']
    if not os.path.exists(f'{os.getcwd()}\\files\\{group}'):
        os.mkdir(f'{os.getcwd()}\\files\\{group}\\')

    with open(f'{os.getcwd()}\\Files\\schedule_{group}.json', 'r') as schedule:
    ...
    for subgroup in range(1, subgroups + 1): 
        ...
        if not os.path.exists(f'{os.getcwd()}\\files\\{group}\\s{subgroup}'):
            os.mkdir(f'{os.getcwd()}\\files\\{group}\\s{subgroup}')
        for quality in qualities:
            if not os.path.exists(f'{os.getcwd()}\\files\\{group}\\s{subgroup}\\{quality}'):
                os.mkdir(f'{os.getcwd()}\\files\\{group}\\s{subgroup}\\{quality}')
            for delta in range(0, max_delta + 1, 7):
                ...
                # dict_file = file_input(file_path=f'{os.getcwd()}\\Files\\schedule_{group}.json', days=dates, subgroup=subgroup)
                ...
                for mode in modes:
                    ...
                    if not os.path.exists(f'{os.getcwd()}\\files\\{group}\\s{subgroup}\\{quality}\\{mode}'):
                        os.mkdir(f'{os.getcwd()}\\files\\{group}\\s{subgroup}\\{quality}\\{mode}')
                    output_path = f'{os.getcwd()}\\Files\\{group}\\s{subgroup}\\{quality}\\{mode}\\week__{dates[0]}_{dates[-1]}.jpg' 
